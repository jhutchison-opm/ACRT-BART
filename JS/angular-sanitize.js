 ...
 */
  ...
  const START_TAG_REGEXP =
    /^<\s*([\w:-]+)((?:\s+[\w:-]+(?:\s*=\s*(?:(?:"[^"]*")|(?:'[^']*')|[^>\s]+))?)*)\s*(\/?)\s*>/;
  const END_TAG_REGEXP = /^<\s*\/\s*([\w:-]+)[^>]*>/;
  const ATTR_REGEXP =
    /([\w:-]+)(?:\s*=\s*(?:(?:"((?:[^"])*)")|(?:'((?:[^'])*)')|([^>\s]+)))?/g;
  const BEGIN_TAG_REGEXP = /^</;
  const BEGING_END_TAGE_REGEXP = /^<\s*\//;
  const COMMENT_REGEXP = /<!--(.*?)-->/g;
  const CDATA_REGEXP = /<!\[CDATA\[(.*?)]]>/g;
  const URI_REGEXP = /^((ftp|https?):\/\/|mailto:|#)/;
  const NON_ALPHANUMERIC_REGEXP = /([^#-~| |!])/g; // Match everything outside of normal chars and " (quote character)
  ...
  const optionalEndTagBlockElements = makeMap(
 ...
 */
  angular.module("ngSanitize").filter("linky", function () {
    ...
    return function (text) {
      ...
      const writer = htmlSanitizeWriter(html);
      let url;
      let i;

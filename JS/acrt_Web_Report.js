const app = angular.module("acrt", ["ngSanitize"]);
...
function expandCollapse4() {
  const z = document.getElementById("expandCollapse4");
  if (
    z.innerHTML ===
    '<i class="down"></i> Hide terms used in the Conformance Level'
  ) {
...
function expandCollapse5() {
  const z = document.getElementById("expandCollapse5");
...
function expandCollapse6() {
  const z = document.getElementById("expandCollapse6");
...
function grpImpact() {
  const x = document.getElementById("myImpact");
...
app.controller("acrtFormCtrl", function ($scope, $filter) {
  ...
  $scope.displayTestResult = function () {
    ...
    if ($scope.selected_name_tstrsltdsply == "All") {
      $scope.displaytstRslt = true;
      $scope.filterResult1 = true;
    }
    if ($scope.selected_name_tstrsltdsply == "Pass") {
      $scope.displaytstRslt = true;
      $scope.filterResult = "Pass";
    }
    if ($scope.selected_name_tstrsltdsply == "Fail") {
      $scope.displaytstRslt = true;
      $scope.filterResult = "Fail";
    }
    if ($scope.selected_name_tstrsltdsply == "Does Not Apply") {
      $scope.displaytstRslt = true;
      $scope.filterResult = "Does Not Apply";
    }
    if ($scope.selected_name_tstrsltdsply == "Not Tested") {
      $scope.displaytstRslt = true;
      $scope.filterResult = "Not Tested";
    }
  };
  ...
  $scope.fileNameChanged = function () {
    ...
    $scope.$apply();
    document.getElementById("selMsg").innerHTML =
      "File selected. Please select 'Load File' below to proceed.";
    document
  ...
  $scope.tstRsltMsg = function () {
    ...
    if ($scope.checked1 == "false") {
      document.getElementById("msg").innerHTML =
        "Check checkbox for detailed Test Results.";
    }
    if ($scope.checked1 == "true") {
      document.getElementById("msg").innerHTML =
        "Uncheck checkbox to hide Test Results.";
    }
  };
  ...
  $scope.zoom = function (i) {
    ...
    const span = document.getElementById("cls" + i);
  ...
  $scope.zoom2 = function (i) {
    const modal2 = document.getElementById("rmd2" + i);
    const img2 = document.getElementById("image2" + i);
    const modalImg2 = document.getElementById("img2" + i);
    const captionText2 = document.getElementById("caption2" + i);
    // var span = document.getElementsByClassName("close")[i];
    const span2 = document.getElementById("cls2" + i);
  ...
  $scope.loadFile = function loadFile() {
    ...
    if (!input) {
      ...
      alert(
        "Please use browser that supports the `files` property of file inputs.",
      );
    ...
    $scope.countOfWords = function (text) {
      const s = text ? text.split(/\s+/) : 0; // it splits the text on space/tab/enter
      return s ? s.length : "";
    };
    ...
    function receivedText(e) {
      ...
      $scope.altforImg = [];
      // KE: BART
      $scope.baselineAligned =
        $scope.jsonData[0].BaselineAlignment.BaselineAligned;
      // alert($scope.baselineAligned);
      if ($scope.baselineAligned == "No") {
        document.getElementById("alignmentresult").innerHTML =
          "<strong>Based on the results for the test cases, " +
          $scope.productID +
          " version " +
          $scope.versionID +
          " does not align with the Baseline for Web. </strong>";
      } else {
        document.getElementById("alignmentresult").innerHTML =
          "<strong>Based on the results for the test cases, " +
          $scope.productID +
          " version " +
          $scope.versionID +
          " aligns with the Baseline for Web. </strong>";
      }
      ...
      for (let b = 0; b < $scope.jsonData[0].Criteria.length; b++) {
        ...
        if ($scope.TestResult[b] != "undefined") {
          $scope.noResult[b] = true;
        }
        if ($scope.TestResult[b] == "undefined") {
          $scope.TestResult[b] = "";
          $scope.noResult[b] = false;
        }
        ...
        $scope.RemediationDate[b] =
          $scope.jsonData[0].Criteria[b].RemediationDate;
        $scope.RemediationDetails[b] =
        ...
        if ($scope.jsonData[0].Criteria[b].TestResult == "undefined")
          $scope.jsonData[0].Criteria[b].TestResult = "";
        ...
        if ($scope.isDraft == true) {
          $scope.draftMsg =
            "This is a Draft Report, To view final Report please select all required test results.";
        }
      ...
      document.getElementById("msg1").innerHTML =
        ...
        $scope.versionID +
        ".json" +
        "</strong> file load completed.<br> To load a different file, <strong>reload</strong> this page.";
      alert(
        "To save printer-friendly HTML report, use Alt+S or Save button at the bottom of the page.",
      );
      ...
      if ($scope.jsonData[0].SuccessCriteria == undefined) {
        ...
        alert(
          'Either select different file (by refreshing this page) or use "Create Report" option for same file.',
        );
      }
      ...
      if ($scope.sucCrtLngth == 0) {
        $scope.wcagRprt = false;
        alert(
          "Make sure to load file with all mandatory fields and at least one valid test result to view complete report page.",
        );
      }
      ...
      for (let a = 0; a < $scope.sucCrtLngth; a++) {
        if (
          $scope.jsonData[0].SuccessCriteria[a].DisabilityImpact == undefined ||
          $scope.jsonData[0].SuccessCriteria[a].DisabilityImpact == "undefined"
        ) {
        ...
        if (
          ...
          {
            $scope.DisabilityImpactCollection.push(
              $scope.jsonData[0].SuccessCriteria[a].DisabilityImpact,
            );
      ...
      // if($scope.DisabilityImpactCollection.length >0){
      $scope.TotalImpactedGrpString = Array.from(
        new Set($scope.DisabilityImpactCollection),
      ).toString();
      ...
      $scope.TotalImpactedGrpString = $scope.TotalImpactedGrpString.replace(
        /^,/,
        "",
      ); // removes first comma from string
      $scope.TotalImpactedGrpString = $scope.TotalImpactedGrpString.replace(
        /,\s*$/,
        " ",
      ); // removes comma from last of string
      $scope.TotalImpactedGrpString = $scope.TotalImpactedGrpString.replace(
        /,+/g,
        ",",
      ); // removes multiple commas from string
      $scope.TotalImpactedGrpString = $scope.TotalImpactedGrpString.replace(
        /,/g,
        "",
      ); // removes all commas
      ...
      $scope.DisabilityRiskScoreJSON = $scope.countOfRepWords(
        $scope.TotalImpactedGrpString,
      );
      $scope.DisabilityRiskScore = $scope.DisabilityRiskScoreJSON;
      // Vision
      $scope.TotalVision = 0;
      if ($scope.DisabilityRiskScoreJSON.Vision == null)
        $scope.DisabilityRiskScoreJSON.Vision = 0;
      $scope.Vision = $scope.DisabilityRiskScoreJSON.Vision;
      if ($scope.DisabilityRiskScoreJSON.VisionWith == null)
        $scope.DisabilityRiskScoreJSON.VisionWith = 0;
      $scope.VisionWith = $scope.DisabilityRiskScoreJSON.VisionWith;
      if ($scope.DisabilityRiskScoreJSON.VisionWithout == null)
        $scope.DisabilityRiskScoreJSON.VisionWithout = 0;
      $scope.VisionWithout = $scope.DisabilityRiskScoreJSON.VisionWithout;
      if ($scope.DisabilityRiskScoreJSON.Color == null)
        $scope.DisabilityRiskScoreJSON.Color = 0;
      $scope.Color = $scope.DisabilityRiskScoreJSON.Color;
      if ($scope.DisabilityRiskScoreJSON.ColorWithout == null)
        $scope.DisabilityRiskScoreJSON.ColorWithout = 0;
      $scope.ColorWithout = $scope.DisabilityRiskScoreJSON.ColorWithout;
      $scope.TotalVisionList =
        "Without Vision (" +
        $scope.VisionWith +
        "), With Limited Vision (" +
        $scope.VisionWithout +
        ...
        ")";
      $scope.TotalVision =
        $scope.VisionWith + $scope.VisionWithout + $scope.Color;

      // Hearing
      $scope.HearingTotal = 0;

      if ($scope.DisabilityRiskScoreJSON.Hearing == null)
        $scope.DisabilityRiskScoreJSON.Hearing = 0;
      $scope.Hearing = $scope.DisabilityRiskScoreJSON.Hearing;
      if ($scope.DisabilityRiskScoreJSON.HearingWith == null)
        $scope.DisabilityRiskScoreJSON.HearingWith = 0;
      $scope.HearingWith = $scope.DisabilityRiskScoreJSON.HearingWith;
      if ($scope.DisabilityRiskScoreJSON.HearingWithout == null)
        $scope.DisabilityRiskScoreJSON.HearingWithout = 0;
      $scope.HearingWithout = $scope.DisabilityRiskScoreJSON.HearingWithout;
      $scope.HearingTotalList =
        "Without Hearing  (" +
        $scope.HearingWith +
        ")" +
        ", With Limited Hearing (" +
        $scope.HearingWithout +
        ")";
      $scope.HearingTotal = $scope.HearingWith + $scope.HearingWithout;

      // Cognitive
      $scope.TotalCognitive = 0;
      if ($scope.DisabilityRiskScoreJSON.Cognitive == null)
        $scope.DisabilityRiskScoreJSON.Cognitive = 0;
      $scope.Cognitive = $scope.DisabilityRiskScoreJSON.Cognitive;
      if ($scope.DisabilityRiskScoreJSON.Learning == null)
        $scope.DisabilityRiskScoreJSON.Learning = 0;
      $scope.Learning = $scope.DisabilityRiskScoreJSON.Learning;
      if ($scope.DisabilityRiskScoreJSON.Speech == null)
        $scope.DisabilityRiskScoreJSON.Speech = 0;
      $scope.Speech = $scope.DisabilityRiskScoreJSON.Speech;
      if ($scope.DisabilityRiskScoreJSON.Language == null)
        $scope.DisabilityRiskScoreJSON.Language = 0;
      $scope.Language = $scope.DisabilityRiskScoreJSON.Language + $scope.Speech;
      $scope.TotalCognitiveList =
      ...
      if ($scope.DisabilityRiskScoreJSON.Photosensitive == null)
        $scope.DisabilityRiskScoreJSON.Photosensitive = 0;
      $scope.Photosensitive = $scope.DisabilityRiskScoreJSON.Photosensitive;
      if ($scope.DisabilityRiskScoreJSON.AbilitiesPhotosensitive == null)
        $scope.DisabilityRiskScoreJSON.AbilitiesPhotosensitive = 0;
      $scope.AbilitiesPhotosensitive =
        $scope.DisabilityRiskScoreJSON.AbilitiesPhotosensitive;
      if ($scope.DisabilityRiskScoreJSON.Seizure == null)
        $scope.DisabilityRiskScoreJSON.Seizure = 0;
      $scope.Mobility = $scope.DisabilityRiskScoreJSON.Seizure;
      if ($scope.DisabilityRiskScoreJSON.Epilepsy == null)
        $scope.DisabilityRiskScoreJSON.Epilepsy = 0;
      $scope.Epilepsy = $scope.DisabilityRiskScoreJSON.Epilepsy;
      $scope.TotalPhotosensitive = $scope.Photosensitive;
      $scope.TotalPhotosensitiveList =
        "Photosensitive Epilepsy / Photosensitive Seizure Disorders (" +
        $scope.Photosensitive +
        ")";

      // MObility
      $scope.TotalMobility = 0;
      if ($scope.DisabilityRiskScoreJSON.Manipulation == null)
        $scope.DisabilityRiskScoreJSON.Manipulation = 0;
      $scope.Manipulation = $scope.DisabilityRiskScoreJSON.Manipulation;
      if ($scope.DisabilityRiskScoreJSON.ManipulationWithout == null)
        $scope.DisabilityRiskScoreJSON.ManipulationWithout = 0;
      $scope.ManipulationWithout =
        $scope.DisabilityRiskScoreJSON.ManipulationWithout;
      $scope.TotalMobility = $scope.Manipulation;
      $scope.TotalMobilityList =
        "With Limited Manipulation (" + $scope.Manipulation + ")";
      $scope.TotalImpactedGroupNo = 0;
      $scope.TotalImpactedGroupNo =
        $scope.TotalMobility +
        $scope.TotalPhotosensitive +
        $scope.TotalCognitive +
        $scope.HearingTotal +
        $scope.TotalVision;
      if ($scope.isDraft == true) {
        $scope.TotalImpactedGroupNo =
          $scope.TotalImpactedGroupNo +
          " (Final score will be determined upon completion of testing.)";
      }
	...
	*/
      if ($scope.isDraft == true) {
        $scope.DisabilityRiskScore =
          $scope.DisabilityRiskScore +
          "  - Select all  test results to get accurate score";
      }
      ...
      $scope.DisabilityImpactCollection =
        $scope.DisabilityImpactCollection.filter($scope.onlyUnique);

      $scope.DisabilityImpactCollection = Array.from(
        new Set($scope.DisabilityImpactCollection),
      );
      $scope.DisabilityImpactCollectionString = "";
      $scope.DisabilityImpactCollectionString =
        $scope.DisabilityImpactCollection.toString();
      $scope.DisabilityImpactCollectionString = Array.from(
        new Set($scope.DisabilityImpactCollectionString.split(",")),
      ).toString();
      $scope.DisabilityImpactCollectionString =
        $scope.DisabilityImpactCollectionString.replace(/(^\s*)|(\s*$)/gi, ""); // removes start and end spaces from string
      $scope.DisabilityImpactCollectionString =
        $scope.DisabilityImpactCollectionString.replace(/[ ]{2,}/gi, " "); // reduces multiple spaces to single space
      $scope.DisabilityImpactCollectionString =
        $scope.DisabilityImpactCollectionString.replace(/^,/, ""); // removes first comma from string
      $scope.DisabilityImpactCollectionString =
        $scope.DisabilityImpactCollectionString.replace(/,\s*$/, " "); // removes comma from last of string
      $scope.DisabilityImpactCollectionString =
        $scope.DisabilityImpactCollectionString.replace(/,+/g, ","); // removes multiple commas from string
      $scope.DisabilityImpactCollectionString =
        $scope.DisabilityImpactCollectionString.replace(/'/g, ""); // removes single quotes
      ...
      if ($scope.DisabilityImpactCollection.length > 0) {
        ...
        if ($scope.isDraft == true) {
          $scope.DisabilityImpactCollection =
            $scope.DisabilityImpactCollection +
            "  - Select all test results to get accurate impacted group";
        }
      } else {
  ...
  $scope.saveHtml = function () {
    setTimeout(function () {
      alert(
        "Please keep  both JSON & HTML files for future reference.Printer-friendly HTML report has been saved to your Downloads folder (unless otherwise specified).",
      );
    }, 6000);
    $scope.saveHtmlIsClickedd = true;

    // This creates WCAG report
    let WCAG = '<table class="table2" > <caption>WCAG Report </caption>';

    WCAG += "<tr>";
    WCAG +=
      '<th scope="col"  title="Criteria"  width="98px">' + "Criteria" + "</th>";
    WCAG +=
      '<th scope="col"  title="Conformance Level" width="218px">' +
      "Conformance Level" +
      "</th>";
    WCAG +=
      '<th scope="col"  title="Guideline" width="450px">' +
      "Standard/Guideline" +
      "</th>";
    WCAG +=
      '<th scope="col"  title="Remarks and Explanations" width="398px">' +
    ...
    for (let i = 0; i < $scope.jsonData[0].SuccessCriteria.length; i++) {
      ...
      WCAG += "<tr >";
      WCAG +=
        '<th scope="row" width="100px" title="Criteria">' +
        $scope.jsonData[0].SuccessCriteria[i].CrtID;
      +"</th>";
      WCAG +=
        '<td  width="220px" title="Conformance Level">' +
        $scope.jsonData[0].SuccessCriteria[i].ConformanceLvl;
      +"</td>";
      WCAG +=
        '<td width="450px" title="Guideline">' +
        $scope.jsonData[0].SuccessCriteria[i].Guideline;
      +"</td>";
      WCAG +=
        '<td width="400px" title="Remarks and Explanations">' +
        $scope.jsonData[0].SuccessCriteria[i].RemarkExplntn;
      +"</td>";
    ...
    let testResult = '<table class="table1" > <caption> Test Results</caption>';

    testResult += "<tr>";
    testResult +=
      '<th scope="col"  title="Issue No." width="50px">' +
      "Issue No." +
      "</th>";
    testResult +=
      '<th scope="col"  title="Test Name"  width="60px">' +
      "Test Name" +
      "</th>";
    testResult +=
      '<th scope="col"  title="Test ID" width="35px">' + "Test ID" + "</th>";
    testResult +=
      '<th scope="col"  title="Test Condition" width="120px">' +
      "Test Condition" +
      "</th>";
    testResult +=
      '<th scope="col"  title="Criteria ID" width="40px">' +
      "Criteria ID" +
      "</th>";
    // testResult += "<th scope=\"col\"  title=\"Test\" width=\"60px\">" + "Test" + "</th>";
    testResult +=
      '<th scope="col"  title="Test Result" width="40px">' +
      "Test Result" +
      "</th>";
    testResult +=
      '<th scope="col" title="Tester\'s Comment" width="140px">' +
      "Tester Comments" +
      "</th>";
    testResult +=
      '<th scope="col"  title="Location/Screen" width="140px">' +
      "Location/Screen" +
      "</th>";
    testResult +=
      '<th scope="col"  title="Browser Type" width="80px">' +
      "Browser Type" +
      "</th>";
    testResult +=
      '<th scope="col"  title="Browser Versions" width="80px">' +
      "Browser Ver." +
      "</th>";
    testResult +=
      '<th scope="col" title="Screenshot" >' + "Screenshot" + "</th>";
    testResult +=
      '<th scope="col"  title="Global Issue" width="40px">' +
      "Global Issue" +
      "</th>";
    testResult +=
      '<th scope="col" title="Remediation Details" width="140px">' +
      "Remediation Details" +
      "</th>";
    testResult +=
      '<th scope="col" title="Screenshot" >' +
      "Remediation Screenshot" +
      "</th>";
    testResult +=
      '<th scope="col"  title="Remediation Date" width="60px">' +
      "Remediation Date" +
      "</th>";
    testResult += "</tr>";

    for (let i = 0; i < $scope.jsonData[0].Criteria.length; i++) {
      if ($scope.jsonData[0].Criteria[i].TestResult == "undefined")
        $scope.jsonData[0].Criteria[i].TestResult = "";
      const d = i + 1;
      // if($scope.noResult[i]== true && $scope.TestResult[i]== $scope.filterResult){
      if ($scope.TestResult[i] == $scope.filterResult) {
        testResult += "<tr >";
        testResult +=
          '<th  title="Issue Number"> Issue ' +
          $scope.jsonData[0].Criteria[i].Counter;
        +"</th>";
        testResult +=
          '<th   scope="row" title="Test Name">' +
          $scope.jsonData[0].Criteria[i].TestName;
        +"</th>";
        testResult +=
          '<td title="Test ID">' + $scope.jsonData[0].Criteria[i].TestID;
        +"</td>";
        testResult +=
          '<td title="Test Condition">' +
          $scope.jsonData[0].Criteria[i].TestCondition;
        +"</td>";
        testResult +=
          '<td title="Success Criteria">' +
          $scope.jsonData[0].Criteria[i].CrtID;
        +"</td>";
        // testResult += "<td title=\"Test\">" + $scope.jsonData[0].Criteria[i].Test; + "</td>";
        testResult +=
          '<td title="Test Result">' +
          $scope.jsonData[0].Criteria[i].TestResult;
        +"</td>";
        testResult +=
          '<td title="Test Process">' +
          $scope.jsonData[0].Criteria[i].TestProcess;
        +"</td>";
        testResult +=
          '<td title="Tester\'s comment">' +
          $scope.jsonData[0].Criteria[i].TesterComment;
        +"</td>";
        testResult +=
          '<td title="Location">' + $scope.jsonData[0].Criteria[i].location;
        +"</td>";
        testResult +=
          '<td title="Browser Type">' +
          $scope.jsonData[0].Criteria[i].T_brwsrType;
        +"</td>";
        testResult +=
          '<td title="Browser\'s Version">' +
          $scope.jsonData[0].Criteria[i].T_brwsrVrsn;
        +"</td>";
        testResult +=
          '<td onclick="zoom(' +
          $scope.jsonData[0].Criteria[i].Counter +
          ...
          '<img id="' +
          $scope.jsonData[0].Criteria[i].Counter +
          '" width="350"  alt="screenshot captured" src= "' +
          $scope.ImageSrc[i] +
          '" ' +
          ...
          "</td>";
        testResult +=
          '<td title="Global Issue">' +
          $scope.jsonData[0].Criteria[i].GlobalIssue;
        +"</td>";
        testResult +=
          '<td title="Remediation Details">' +
          $scope.jsonData[0].Criteria[i].RemediationDetails;
        +"</td>";
        testResult +=
          '<td onclick="zoom(' +
          $scope.jsonData[0].Criteria[i].Counter +
          ...
          "</td>";
        testResult +=
          '<td title="Remediation Date">' +
          $scope.jsonData[0].Criteria[i].RemediationDate;
        +"</td>";
    ...
    let RSCSCORE =
      '<table class="table2" > <caption>Score Breakdown </caption>';

    RSCSCORE += "<tr>";
    RSCSCORE +=
      '<th scope="col"  title="Disability Group"  width="250px">' +
      "Disability Group" +
      "</th>";
    RSCSCORE +=
      '<th scope="col"  title="Risk Score" width="100px">' +
      "Risk Score" +
      "</th>";
    RSCSCORE +=
      '<th scope="col"  title="Detailed Impacted Groups (with Score)" width="550px">' +
      "Detailed Impacted Groups (with Score)" +
      "</th>";
    RSCSCORE += "</tr>";
    RSCSCORE += "<tr >";
    RSCSCORE +=
      '<th scope="row" width="250px" title="Vision">' + "Vision" + "</th>";
    RSCSCORE +=
      '<td  width="100px" title="Risk Score for Vision ">' + $scope.TotalVision;
    +"</td>";
    RSCSCORE +=
      '<td width="550px" title="Detailed Impacted Groups for Vision">' +
      $scope.TotalVisionList;
    +"</td>";
    RSCSCORE += "</tr>";
    ...
    RSCSCORE += "<tr >";
    RSCSCORE +=
      '<th scope="row" width="250px" title="Hearing">' + "Hearing" + "</th>";
    RSCSCORE +=
      '<td  width="100px" title="Risk Score for Hearing">' +
      $scope.HearingTotal;
    +"</td>";
    RSCSCORE +=
      '<td width="550px" title="Detailed Impacted Groups for Hearing">' +
      $scope.HearingTotalList;
    +"</td>";
    RSCSCORE += "</tr>";
    ...
    RSCSCORE += "<tr >";
    RSCSCORE +=
      '<th scope="row" width="250px" title="Cognitive">' +
      "Cognitive" +
      "</th>";
    RSCSCORE +=
      '<td  width="100px" title="Risk Score for Cognitive">' +
      $scope.TotalCognitive;
    +"</td>";
    RSCSCORE +=
      '<td width="550px" title="Detailed Impacted Groups for Cognitive">' +
      $scope.TotalCognitiveList;
    +"</td>";
    RSCSCORE += "</tr>";
    ...
    RSCSCORE += "<tr >";
    RSCSCORE +=
      '<th scope="row" width="250px" title="Photosensitivity">' +
      "Photosensitivity" +
      "</th>";
    RSCSCORE +=
      '<td  width="100px" title="Risk Score for Photosensitivity">' +
      $scope.TotalPhotosensitive;
    +"</td>";
    RSCSCORE +=
      '<td width="550px" title="Detailed Impacted Groups for Photosensitivity">' +
      $scope.TotalPhotosensitiveList;
    +"</td>";
    RSCSCORE += "</tr>";
    ...
    RSCSCORE += "<tr >";
    RSCSCORE +=
      '<th scope="row" width="250px" title="Mobility">' + "Mobility" + "</th>";
    RSCSCORE +=
      '<td  width="100px" title="Risk Score for Mobility">' +
      $scope.TotalMobility;
    +"</td>";
    RSCSCORE +=
      '<td width="550px" title="Detailed Impacted Groups Mobility">' +
      $scope.TotalMobilityList;
    +"</td>";
    RSCSCORE += "</tr>";
    ...
    $scope.capturedFormData =
      ...
      "<title>" +
      $scope.productID +
      " Accessibility Conformance Report  </title>" +
      ...
      "<h1>" +
      $scope.companyname +
      " Accessibility Conformance Report </h1>" +
      // "<span> Based on VPAT® Version 2.4 </span>"+
      '  <h2 id="draftMsg"  style="color: #FFFFFF; background-color: #be0004;" hidden>' +
      $scope.draftMsg +
      "</h2> " +
      " <p>This application was tested according to the Trusted Tester Section 508 Conformance Test method: " +
      $scope.evalMethod +
      " " +
      $scope.evalMethodVrsn +
      ". The review may be a sampling of pages to confirm product compliance. The responsibility for full and complete testing and compliance remains with the owner of the application or website.</p>" +
      "<br><b>Review Date:  &nbsp;  </b>" +
      $scope.dateSubmitted +
      "<br>" +
      "<h2> Process Information </h2>" +
      '<input type="text" hidden id="isDraftValue" value=' +
      $scope.isDraft +
      "> " +
      "<b> Process Name:  &nbsp;  </b>" +
      $scope.productID +
      ...
      $scope.ownerID +
      "<br>" +
      "<b>Process Owner/Vendor:  &nbsp;  </b>" +
      $scope.versionID +
      "<br>" +
      "<b>Process Type:  &nbsp;  </b>" +
      $scope.productType +
      "<br>" +
      /* "<b>Location:  &nbsp;  </b>" + */ $scope.urlID +
      "<br>" +
      "<b>Process Description:  &nbsp;  </b>" +
      $scope.prodDescID +
      "<br>" +
      "<h2> Submitter's Information </h2>" +
      "<strong>Submitter's First Name:  &nbsp;  </strong>" +
      $scope.firstname +
      "<br>" +
      "<strong>Submitter's Last Name:  &nbsp;  </strong>" +
      $scope.lastname +
      "<br>" +
      "<strong>Submitter's Title:  &nbsp;  </strong>" +
      $scope.testerID +
      "<br>" +
      "<strong>Agency Name:  &nbsp;  </strong>" +
      $scope.companyname +
      "<br>" +
      "<strong>Submitter's Email:  &nbsp;  </strong>" +
      $scope.testerContact +
      "<br>" +
      ...
      $scope.evalMethod +
      "<br>" +
      "<strong>Testing Method Version:  &nbsp;  </strong>" +
      $scope.evalMethodVrsn +
      "<br>" +
      ...
      "<h2> Web Content Accessibility Guidelines (WCAG) Report </h2>" +
      WCAG +
      "<br>" +
      "<h2> Test Results </h2>" +
      testResult +
      "<br>" +
      "<h2> Disability Impact Score </h2>" +
      $scope.impctInfoMsg +
      "<br><br> <strong>Risk Score: " +
      $scope.TotalImpactedGroupNo +
      "</strong><br>" +
      RSCSCORE +
      "<br><br>" +
      ...
      0 +
      ']; img.onclick = function() { modal.style.display = "block"; modalImg.src = this.src; captionText.innerHTML = this.alt; }span.onclick = function() { modal.style.display = "none"; } } </script>' +
      "</body>" +
      "</html>";
    const htmlContent = [$scope.capturedFormData];

    const bl = new Blob(htmlContent, {
      type: "text/html",
    });
